...
public class NarTestCompileMojo extends AbstractCompileMojo {
	...
	private void createTest(final Project antProject, final Test test)
		...
		// add linker
		final LinkerDef linkerDefinition = getLinker().getTestLinker(this, task, getOS(),
				getAOL().getKey() + ".linker.", type, linkPaths);
		...
		for (final Object depLib : dependencies) {
			...
			if (!binding.equals(Library.JNI) && !binding.equals(Library.NONE) && !binding.equals(Library.EXECUTABLE)) {
				...
				if (dir.exists()) {
					...
					final File aolNarInfoFile = getLayout().getNarInfoDirectory(getUnpackDirectory(),
	 ...
	 * used to get the include paths needed for compilation and to get the libraries
	 * paths and names needed for linking.
	 */
	@Override
