...
public abstract class AbstractDependencyMojo extends AbstractNarMojo {
	...
	/**
	 * The Repository object that dispatches the verbose dependency graph collection
	 * request.
	 * 
	 * @since 3.5.2
	 */
	...
	/**
	 * The Session object for controling/configuring the verbose dependency graph
	 * collection request.
	 * 
	 * @since 3.5.2
	 */
	...
	/**
	 * Gets the project's full dependency tree prior to dependency mediation. This
	 * is required if we want to know where to push libraries in the linker line.
	 * 
	 * @return {@link org.eclipse.aether.graph.DependencyNode Root node} of the
	...
	protected org.eclipse.aether.graph.DependencyNode getVerboseDependencyTree() {
		...
		org.eclipse.aether.graph.Dependency dep = new org.eclipse.aether.graph.Dependency(
				new org.eclipse.aether.artifact.DefaultArtifact(art.getGroupId(), art.getArtifactId(), null,
						art.getVersion()),
				null);

		// Set the root of the request, in this case the current project will be the
		// root
	...
	/**
	 * Serializes the dependency tree of current maven project to a string of comma
	 * separated list of groupId:artifactId traversing nodes in Level-Order way
	 * (also called BFS algorithm)
	 * 
	 * @param pushDepsToLowestOrder when {@code true} enables linker re-ordering
	 *                              logic such that libraries that are lower in the
	 *                              dependency hierarchy appear lower in the list,
	 *                              maximizing symbol resolution.
	 * @return {@link String Dependency tree string} of comma separated list of
	 *         groupId:artifactId
	...
	protected String dependencyTreeOrderStr(boolean pushDepsToLowestOrder, boolean specifyDirectDeps)
		...
		/*
		 * Check if we should try to push libraries to their lowest place in the link
		 * order. We will use the full dependency tree from Aether (unmediated) to
		...
		if (pushDepsToLowestOrder || specifyDirectDeps) {
			...
			/*
			 * Traverse full dependency list in REVERSE order. The dependencies at the end
			 * of the list signify the ones that are most heavily depended on and therefore
			 * need to occur later in the link order.
			 */
			for (int i = verboseDepList.size() - 1; i >= 0; i--) {
				String depStr = verboseDepList.get(i);

				/*
				 * Create link order by pushing new dep to the front of the list. Do not insert
				 * dep if it was added already (i.e. if adding to ReducedDepSet fails)
				 */
			...
			if (specifyDirectDeps) {
				// Get first level of deps, when specifyAllDirectDeps is true those can be the
				// only ones linked.
	...
	/**
	 * Gets the set strings of the form "&lt;groupId&gt;:&lt;artifactId&gt;"
	 * representing the specified nodes direct dependencies (immediate children).
	 *
	 * @param rootNode {@link org.eclipse.aether.graph.DependencyNode root node} of
	 *                 the tree to fetch the direct dependencies from.
	 * @return {@link HashSet<String>} of the form
	 *         "&lt;groupId&gt;:&lt;artifactId&gt;" containing all direct
	 *         dependencies of the parameter specified by rootNode.
	 * @since 3.5.3
	 */
	...
	/**
	 * Get List of Nodes of Dependency tree serialised by traversing nodes in
	 * Level-Order way (also called BFS algorithm)
	 *
	 * @param rootNode root node of the project Dependency tree
	 * @return the dependency tree string of comma separated list of
	 *         groupId:artifactId
	 */
	...
	/**
	 * helper function for traversing nodes in Level-Order way (also called BFS
	 * algorithm)
	 */
	private List<DependencyNode> levelTraverseTreeList(List<DependencyNode> nodeList,
			List<DependencyNode> aggDepNodeList) {
	...
	/**
	 * Get verbose List of Nodes in Dependency tree serialized by using a BFS
	 * algorithm (copied/refactored from the depLevelOrderList method)
	 *
	 * @param rootNode {@link org.eclipse.aether.graph.DependencyNode root node} of
	 *                 the project's dependency tree
	 * @return {@link List} of strings of the form
	 *         "&lt;groupId&gt;:&lt;artifactId&gt;" representing the verbose (full)
	 *         dependency tree.
	 * @throws MojoExecutionException
	 * @since 3.5.2
	 */
	private List<String> depLevelVerboseList(org.eclipse.aether.graph.DependencyNode rootNode)
			throws MojoExecutionException {

		// Create list to store aggregate of all nodes in the dependency tree BFS
		List<org.eclipse.aether.graph.DependencyNode> AggDepNodeList = new ArrayList<org.eclipse.aether.graph.DependencyNode>();

		// Create list that stores current breadth
		Set<org.eclipse.aether.graph.DependencyNode> NodeChildList = new LinkedHashSet<org.eclipse.aether.graph.DependencyNode>(
		...
		// Construct list of Strings representing the deps in the form
		// "<groupId>:<artifactId>"
	...
	/**
	 * helper function for traversing nodes from the Aether package in Level-Order
	 * way. (copied/refactored from the levelTraverseTreeList method)
	 *
	 * @param nodeList       {@link List} representing current breadth of nodes to
	 *                       be visited
	 * @param aggDepNodeList {@link List} representing aggregate nodes current
	 *                       breadth is added to.
	 * @param rootNode       {@link org.eclipse.aether.graph.DependencyNode root
	 *                       node} of the project dependency tree used to check for
	 *                       circular dependencies.
	 * @return {@link List} of dependency nodes representing the next breadth to
	 *         visit.
	 * @throws MojoExecutionException
	 * @since 3.5.2
	 */
		...
		for (org.eclipse.aether.graph.DependencyNode node : nodeList) {
			if (nodeArtifactsMatch(rootNode, node)) {
				throw new MojoExecutionException(
						"Circular dependency detected in project: " + getMavenProject().toString());
			}
			if ((node != null) && (node.getChildren() != null)) {
				NodeChildList.addAll(node.getChildren());
			}
	...
	/**
	 * Convenience function for constructing a String representing an artifact in
	 * the form "<groupId>:<artifactId>"
	 * 
	 * @param artifact {@link org.eclipse.aether.Artifact artifact} to construct
	 *                 string from
	 * @return {@link String} in the form "<groupId>:<artifactId>" representing the
	 *         artifact
	 * @since 3.5.3
	 */
	...
	/**
	 * Convenience function for determining if the 2 specified dependency nodes'
	 * artifacts match (i.e. if their group and artifact ids match)
	 * 
	 * @param nodeA {@link org.eclipse.aether.graph.DependencyNode dependency node}
	 *              to match against
	 * @param nodeB {@link org.eclipse.aether.graph.DependencyNode dependency node}
	 *              to match against
	 * @return {@code true} if the groupId and artifactId of the provided
	 *         DependencyNode objects match, {@code false} otherwise.
	 * @since 3.5.2
	 */
	 ...
	 * dependencies artifacts of the project. The artifacts which will be processed
	 * are those returned by the method getArtifacts() which must be implemented in
	 * each class which extends AbstractDependencyMojo.
	 * 
		...
		if (nars != null) {
			for (final String nar2 : nars) {
				...
				if (nar.length >= 4) {
					...
					getLog().warn(
							"nars property in " + dependency.getArtifactId() + " contains invalid field: '" + nar2);
				}
			}
		}
		return artifactList;
