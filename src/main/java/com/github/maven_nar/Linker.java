...
public class Linker {
	...
	/**
	 * Adds libraries to the linker. Will work in combination with &lt;libs&gt;. The
	 * format is comma separated, colon-delimited values (name:type:dir), like
	...
	/**
	 * <p>
	 * Specifies the link ordering of libraries that come from nar dependencies. The
	 * format is a comma separated list of dependency names, given as
	 * groupId:artifactId.
	 * </p>
	 * <p>
	...
	/**
	 * <p>
	 * Specifies to use Default link ordering of libraries that come from mvn
	 * dependency tree. The Default link order (generated by nar) is a Level-order
	 * tree traversing list (also called BFS) of dependency tree, given as a comma
	 * separated list of groupId:artifactId.
	 * </p>
	 * <p>
	...
	/**
	 * Specifies that if using default dependency lib order then turn on/off logic
	 * that pushes dependencies to appropriate place in linker line based on
	...
	public final LinkerDef getLinker(final AbstractCompileMojo mojo, final CCTask task, final String os,
		...
		// if No user preference of dependency library link order is specified then use
		// the Default one nar generate.
		if ((this.narDependencyLibOrder == null) && (narDefaultDependencyLibOrder)) {
			if (os.equals(OS.AIX) && (getName(null, null).equals("xlC_r") || getName(null, null).equals("xlC")
				...
				this.narDependencyLibOrder = mojo.dependencyTreeOrderStr(pushDepsToLowestOrder,
						mojo.getDirectDepsOnly());
			}
		} else if (pushDepsToLowestOrder && !narDefaultDependencyLibOrder) {
			mojo.getLog()
					.warn("pushDepsToLowestOrder will have no effect since narDefaultDependencyLibOrder is disabled");
		...
		// Add transitive dependencies to the shared library search path if
		// directDepsOnly is enabled, this is not a static library, and the OS is either
		// Linux or AIX.
		if (linkPaths != null && linkPaths.size() > 0 && mojo.getDirectDepsOnly() && !type.equals(Library.STATIC)
				&& (os.equals(OS.LINUX) || os.equals(OS.AIX))) {
