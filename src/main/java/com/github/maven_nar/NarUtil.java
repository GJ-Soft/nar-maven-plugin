...
public final class NarUtil {
	...
	public static void deleteDirectory(final File dir) throws MojoExecutionException {
		int retries = OS.WINDOWS.equalsIgnoreCase(System.getProperty("os.name")) ? 3 : 1; // Windows file locking (such
																							// as due to virus scanners)
																							// and sometimes deleting
																							// slowly, or slow to report
																							// completion.
		while (retries > 0) {
			retries--;
			try {
...
//      getLog().info("Could not delete directory: " + dir + " : Retrying");
	...
	/**
	 * Recursively deletes a directory tree. Does nothing if the path does not
	 * exist. Symbolic links are removed without following them.
	 */
	...
	public static AOL getAOL(final MavenProject project, final String architecture, final String os,
		...
		/*
		 * To support a linker that is not the default linker specified in the
		 * aol.properties
		 */
		String aol_linker;
	...
	public static void makeExecutable(final File file, final Log log)
		...
		if (file.isDirectory()) {
			try (DirectoryStream<Path> children = Files.newDirectoryStream(file.toPath())) {
				for (final Path child : children) {
					makeExecutable(child.toFile(), log);
				}
			} catch (final IOException e) {
				throw new MojoExecutionException("Could not list directory: " + file, e);
			}
		}
	...
	/**
	 * Replaces every literal occurrence of {@code target} with {@code replacement}
	 * in {@code string}.
	 *
	...
	public static void writeCommandFile(File file, List<String[]> commands) throws MojoExecutionException {
		try (PrintWriter compileCommandWriter = new PrintWriter(Files.newBufferedWriter(file.toPath()))) {
	...
	public static Set<PosixFilePermission> parseOctalPermission(String octal) {
		...
		if ((dec & 0b000000001) != 0)
			permissions.add(PosixFilePermission.OTHERS_EXECUTE);
		if ((dec & 0b000000010) != 0)
			permissions.add(PosixFilePermission.OTHERS_WRITE);
		if ((dec & 0b000000100) != 0)
