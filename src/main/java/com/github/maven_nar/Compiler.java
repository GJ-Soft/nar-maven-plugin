...
public abstract class Compiler {
	...
	/**
	 * To use full path for the filenames. false to have "relative" path true to
	 * have "absolute" path absolute: will give path from filesystem root "/"
	 * relative: will give relative path from "workdir" which is usually after
	 * "${basedir}/src/main"
	 */
	...
	/**
	 * Sets optimization. Possible choices are: "none", "size", "minimal", "speed",
	 ...
	 * where the value is optional. Will work in combination with &lt;defines&gt;.
	 */
	@Parameter
	 ...
	 * where the value is optional. Will work in combination with &lt;undefines&gt;.
	 */
	@Parameter
	...
	/**
	 * Filter elements such as cr\lf that are problematic when used inside a
	 * `define`
	 * 
	...
	public final CompilerDef getCompiler(final String type, final String output)
		...
		if (!this.clearDefaultOptions) {
			final String optionsProperty = NarProperties.getInstance(this.mojo.getMavenProject())
					.getProperty(getPrefix() + "options");
		...
		if (!this.clearDefaultDefines) {
			final DefineSet ds = new DefineSet();
			final String defaultDefines = NarProperties.getInstance(this.mojo.getMavenProject())
					.getProperty(getPrefix() + "defines");
		...
		if (!this.clearDefaultUndefines) {
			final DefineSet us = new DefineSet();
			final String defaultUndefines = NarProperties.getInstance(this.mojo.getMavenProject())
					.getProperty(getPrefix() + "undefines");
	...
	protected final Set<String> getExcludes(final String type) throws MojoFailureException, MojoExecutionException {
		...
		if (type.equals(TEST) && !this.testExcludes.isEmpty()) {
			...
			final String defaultExcludes = NarProperties.getInstance(this.mojo.getMavenProject())
					.getProperty(getPrefix() + "excludes");
	...
	protected final Set<String> getIncludes(final String type) throws MojoFailureException, MojoExecutionException {
		...
		if (!type.equals(TEST) && !this.includes.isEmpty()) {
			...
			final String defaultIncludes = NarProperties.getInstance(this.mojo.getMavenProject())
					.getProperty(getPrefix() + "includes");
