...
public class NarCompileMojo extends AbstractCompileMojo {
	...
	private void copyInclude(final Compiler c) throws IOException, MojoExecutionException, MojoFailureException {
		...
		c.copyIncludeFiles(getMavenProject(), getLayout().getIncludeDirectory(getTargetDirectory(),
	...
	private void createLibrary(final Project antProject, final Library library)
		...
		if (directDepsOnly) {
			...
			// Trim all deps from dependencies that are not in the directDepsSet, warn if
			// they are found.
			while (depsIt.hasNext()) {
				...
				if (!directDepsSet.contains(dep.getGroupId() + ":" + dep.getArtifactId())) {
					...
					depsIt.remove();
					// If this transitive dependency was a shared object, add it to the linkPaths
					// list.
					String depType = dep.getBinding(null, null);
					if (Objects.equals(depType, Library.SHARED)) {
						File soDir = getLayout().getLibDirectory(getTargetDirectory(), dep.getArtifactId(),
								dep.getVersion(), getAOL().toString(), depType);
						if (soDir.exists()) {
			...
			for (final NarArtifact dependency : depLibs) {
				...
				if (!binding.equals(Library.JNI) && !binding.equals(Library.NONE)
					...
					if (dir.exists()) {
						...
						final File aolNarInfoFile = getLayout().getNarInfoDirectory(unpackDirectory,
								dependency.getGroupId(), dependency.getArtifactId(), dependency.getBaseVersion(),
		...
		if (getOS().equals(OS.WINDOWS) && Library.STATIC.equals(library.getType())) { // option? should debug symbols
			...
			try {
				NarUtil.copyDirectoryStructure(task.getObjdir(), task.getOutfile().getParentFile(), "**/*.pdb",
	...
	/**
	 * List the dependencies needed for compilation, those dependencies are used to
	 * get the include paths needed for compilation and to get the libraries paths
	 * and names needed for linking.
	 */
	@Override
	...
	private List<File> getSourcesFor(final Compiler compiler) throws MojoFailureException, MojoExecutionException {
		...
		try {
			...
			final List<File> srcDirs = compiler.getSourceDirectories();
			getLog().debug("* Source directories ----------------------------------------------");
			srcDirs.stream().forEach(f -> getLog().debug(String.format("  > %s", f.getAbsolutePath())));
			getLog().debug("-------------------------------------------------------------------");
			for (final File dir : srcDirs) {
				if (dir.exists()) {
					files.addAll(
							FileUtils.getFiles(dir, StringUtils.join(compiler.getIncludes().iterator(), ","), null));
				}
			}
			getLog().debug("* Source files ----------------------------------------------------");
			files.stream().forEach(f -> getLog().debug(String.format("  > %s", f.getName())));
			getLog().debug("-------------------------------------------------------------------");
			return files;
